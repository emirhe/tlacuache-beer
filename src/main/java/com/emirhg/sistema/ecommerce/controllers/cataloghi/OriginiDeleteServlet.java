...
@WebServlet(name = "OriginiDeleteServlet", urlPatterns = {"/OriginiDeleteServlet"})
public class OriginiDeleteServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if (id == null || id <= 0) {
                ...
                response.sendRedirect(request.getContextPath() + "/OrigineListServlet");
                return;
            }           
                        
            boolean ok = origineDao.deleteById(id);
            if (ok) {
                request.getSession().setAttribute("success", "Origine con id " + id + " eliminata!");
            } else {
        ...
        response.sendRedirect(request.getContextPath() + "/OrigineListServlet?action=listar");
